...
class PythonOperator(BaseOperator):
    ...
    def determine_op_kwargs(python_callable: Callable,
        ...
        Function that will inspect the signature of a python_callable to determine which values
...
class _PythonDecoratedOperator(BaseOperator):
    ...
    def _get_unique_task_id(task_id: str, dag: Optional[DAG] = None) -> str:
        ...
        Generate unique task id given a DAG (or if run in a DAG context) Ids are generated by
        appending a unique number to the end of the original task id.
...
class ShortCircuitOperator(PythonOperator, SkipMixin):
    ...
    Allows a workflow to continue only if a condition is met. Otherwise, the workflow "short-circuits"
...
class PythonVirtualenvOperator(PythonOperator):
    ...
    Allows one to run a function in a virtualenv that is created and destroyed automatically
...
def get_current_context() -> Dict[str, Any]:
    ...
    Obtain the execution context for the currently executing operator without altering user
    method's signature.
